...
In todays lab we have FurHire an app that let's you sign up as a pet looking to provide services, or a recruiter looking to hire a new pet. As any good tester would do, make an account for each and explore both roles, and also how they interact with each other.
...
- We now see we have applicants for this position and we really don't since we just created it, plus our database error is gone.

I'll save you some of the pain I went through here, but `orderby` doesn't play well, and the back end strips commas.
...
```
- It's important that we use `GLOB` here and not `LIKE` since `GLOB` is case sensitive and will get proper results when extracting our flag with `a-z` `A-Z` and `0-9`

![FurHire8](/assets/images/Pasted%20image%2020260126183206.png)

I learned after the fact that you can get a `UNION` operator to do some work here even with the commas being stripped it would just take some trial and error on your part since `orderby` is no help we would just keep increasing the number of `JOIN` clauses. We could also look for the number of fields returned in a normal application response and get a close estimate to how many we would need.:
```sql
