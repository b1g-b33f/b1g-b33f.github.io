...
XSS triggered through the recruiter-controlled `reason` field when updating a job application status. The payload is stored server-side and broadcast to the target applicant via a Socket.io `status_update` event. The client-side `showToast()` function renders the message via `innerHTML` without sanitization, allowing arbitrary script execution in the victim's browser context.
...
`#bugforge` `#furhire` `#xss` `#websocket` `#stored-xss` `#filter-bypass` `#mixed-encoding` `#html-entities` `#csrf-via-xss`
